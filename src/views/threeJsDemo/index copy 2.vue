...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$refs.three_demo.appendChild(this.stats.domElement);
      ...
      for (let i = 0; i < this.NUM_SPHERES; i++) {
        ...
        this.spheres.push({
          mesh: sphere, //网格模型
          collider: new THREE.Sphere( //球体的对象，用于碰撞（三维空间中球体的中心坐标，球体的半径）
            new THREE.Vector3(0, -100, 0),
            this.SPHERE_RADIUS
          ),
          velocity: new THREE.Vector3(), //速度矢量
        });
      }
    },
    getForwardVector() {
    ...
    throwBall() {
      ...
      // 返回一个能够表示当前摄像机所正视的世界空间方向的Vector3对象 赋值给playerDirection
      this.camera.getWorldDirection(this.playerDirection);
      sphere.collider.center
        .copy(this.playerCollider.end) //将playerCollider.end复制给球心
        .addScaledVector(
      ...
      // sphere.velocity.copy(this.playerDirection).multiplyScalar(30); //.multiplyScalar ( s : Float ) : this 将该向量与所传入的标量s进行相乘。
      sphere.velocity.addScaledVector(this.playerVelocity, 2); //.addScaledVector ( v : Vector3, s : Float ) : this 将所传入的v与s相乘所得的乘积和这个向量相加。
